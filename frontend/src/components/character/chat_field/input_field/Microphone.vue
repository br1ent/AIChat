...
const emit = defineEmits(["close", "stop", "send"]);
...
<template>
  <div class="absolute bottom-4 left-2 h-12 w-86 flex items-center bg-black/30 backdrop-blur-sm rounded-2xl">
    ...
    <div v-else class="text-white/50 w-full text-base text-center">
      语音输入...
    </div>
    <div class="flex justify-center items-center absolute right-2 w-8 h-8 cursor-pointer" @click="emit('close')">
      <KeyboardIcon />
    </div>
  </div>
</template>
