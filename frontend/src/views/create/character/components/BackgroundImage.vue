...
async function openModal(photo) {
  ...
  if (!croppie) {
    ...
    const viewportWidth = isSmallScreen ? Math.min(window.innerWidth - 96, 240) : 300;
    const viewportHeight = Math.round(viewportWidth * 5 / 3);
    const boundaryWidth = isSmallScreen ? Math.min(window.innerWidth - 48, 320) : 600;
    const boundaryHeight = isSmallScreen ? Math.min(window.innerHeight - 200, 520) : 600;
    croppie = new Croppie(croppieRef.value, {
        viewport: {width: viewportWidth, height: viewportHeight}, // 裁剪框的长宽和样式
        boundary: {width: boundaryWidth, height: Math.max(boundaryHeight, viewportHeight + 40)}, // 外边框的长宽
...
<template>
  <fieldset class="fieldset">
    ...
    <div class="avatar relative">
      ...
      <div v-else class="w-15 h-25 bg-base-200 rounder-box"></div>
  ...
  <input type="file" accept="image/*" class="hidden" ref="file-upload-ref" @change="onFileChange">
    ...
    <div class="modal-box transition-none max-w-[calc(100vw-1rem)] sm:max-w-2xl">
